...
CollabEdit does not use `y-webrtc`. Cross-device WebRTC still needs a rendezvous mechanism so browsers can exchange SDP and ICE information before the direct connection exists. CollabEdit uses Trystero's decentralized Nostr strategy for this discovery step instead of depending on a single Yjs signaling server.

The discovery relays never carry the shared document. After two browsers discover one another, Yjs document state, incremental updates, presence, and cursor information are sent directly over the WebRTC data channel. Trystero encrypts the discovery/session data and WebRTC transport is encrypted end to end.
...
- **Finding peers…** — decentralized discovery is starting.
- **Waiting for peer** — at least one discovery relay is reachable, but nobody else is connected to the room.
- **Connected · N online** — one or more direct WebRTC peers are connected.
- **Discovery unavailable** — no discovery relay became reachable within the timeout.
...
- a decentralized relay/tracker network,
...
- Peer discovery currently depends on Trystero's public decentralized Nostr relay network, but CollabEdit does not operate or require its own backend.
